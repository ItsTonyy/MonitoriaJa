  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (valido) {
      if (opcaoMonitor === null) {
        ...
        try {
          const urlAvatar = fileAvatar
            ? await uploadArquivo(fileAvatar)
          ...
          // adicionar o monitor em todas as disciplinas selecionadas
          if (monitorCriado?.id) {
            const disciplinaIds = listaEspecialidades.map((d) => d._id).filter(Boolean) as string[];
            if (disciplinaIds.length > 0) {
              await Promise.all(
                disciplinaIds.map((id) => adicionarMonitorNaDisciplina(id, monitorCriado.id))
              );
            }
          }
    ...
    <Box>
      <Box sx={{ justifySelf: 'center' }}>
        ...
        <Avatar
          ...
          src={avatar || ''}
        />
        <Button component='label' role={undefined} variant='contained' tabIndex={-1} startIcon={<AddAPhotoIcon />}>
          Insira sua foto
          <VisuallyHiddenInput type='file' accept='image/*' onChange={mudarAvatar} />
        </Button>
      </Box>
        ...
        {opcaoMonitor === null || opcaoMonitor === false ? (
          ...
          <>
            <FormControl error={listaEspecialidades.length === 0} sx={{ width: '100%' }} variant='outlined'>
              ...
              {listaEspecialidades.length === 0 && <FormHelperText>Selecione uma ou mais disciplinas</FormHelperText>}
              <TextField
